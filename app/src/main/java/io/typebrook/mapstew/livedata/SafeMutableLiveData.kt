...
open class SafeMutableLiveData<T>(value: T) : LiveData<T>(value) {
    ...
    public override fun setValue(value: T) =
        if (predicate(value) && value != this.value) super.setValue(transformer(value)) else Unit

    public override fun postValue(value: T) =
        if (predicate(value) && value != this.value) super.postValue(transformer(value)) else Unit
